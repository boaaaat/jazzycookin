...
public final class BlenderSimulationDomain implements StationSimulationDomain {
    ...
    public boolean supports(StationSimulationAccess access) {
        return access.simulationStationType() == StationType.BLENDER
    ...
    private static ItemStack previewOutput(StationSimulationAccess access) {
        ...
        if (analysis.hasTrait(FoodTrait.DAIRY)
                && (analysis.hasTrait(FoodTrait.FRUIT) || analysis.hasTrait(FoodTrait.VEGETABLE))) {
            return CompositionalSimulationSupport.directPreparedOutput(
                    access,
                    analysis,
                    matter,
                    JazzyItems.SMOOTHIE_BLEND.get(),
    ...
    private static FoodMatterData previewMatter(StationSimulationAccess access) {
        ...
        float completion = access.simulationActive() && access.simulationMaxProgress() > 0
                ...
                : 0.86F;
        return CompositionalSimulationSupport.composeMatter(access, analysis, IngredientState.SMOOTH, false, completion,
